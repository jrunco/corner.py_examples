    ...
    **hist2d_kwargs
    ...
    for i, x in enumerate(xs):
        ...
        if len(quantiles) > 0:
            ...
            for q in qvalues:
                if i == 0:
                    ...
                    ax.axhline(q, ls="dashed", color=color)
            if verbose:
                print("Quantiles:")
        ...
        if i == 0:
            ...
            _set_ylim(new_fig, ax, range[i])
            if scale_hist:
                maxn = np.max(n)
                _set_xlim(new_fig, ax, [-0.1 * maxn, 1.1 * maxn])

            else:
                _set_xlim(new_fig, ax, [0, 1.1 * np.max(n)])
